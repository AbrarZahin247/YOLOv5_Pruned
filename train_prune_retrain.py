...
def train(hyp, opt, device, callbacks):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        if(epoch==opt.prune_epoch):
            prune(model=model,amount=opt.pruning_ratio)
...
def parse_opt(known=False):
    ...
    parser.add_argument("--prune-epoch", type=int, default=10, help="total training epochs")
    parser.add_argument("--pruning-ratio", type=float, default=0.8, help="total training epochs")
